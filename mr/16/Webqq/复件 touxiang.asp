...
OnLineUserIDAry = OnLineUserID.split(",")
			//alert(window.parent.document.all.faceImg.length);
if(window.parent.document.all.faceImg){
if(window.parent.document.all.faceImg.length!=undefined){
	if(window.parent.document.all.faceImg.length){
	 ...
	 var IsOnline = "0"
		for(j=0;j<OnLineUserIDAry.length;j++){
	   	...
	   	if(IsOnline=="1"){
		 	window.parent.document.all.faceImg.src=window.parent.document.all.faceImg.src;
	   	}
	   	else{
