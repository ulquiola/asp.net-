...
if request.Form("HqyNO")<>"" then
	...
	UP="Update DB_KhInfo set qyname="&HqyNO &" where khnumber='"&session("khnumber")&"'"
	conn.execute(UP)%>
...
session("khnumber")=request("khnumber")
end if
...
If (Session("khnumber") <> "") Then 
  hyNO = Session("khnumber")
End If
Set rs_kh = Server.CreateObject("ADODB.Recordset")
sql_kh = "SELECT khnumber, khname, qyname FROM DB_KhInfo WHERE khnumber = '" &_
 Replace(hyNO, "'", "''") & "'"
...
window.location.href="areamodify_KH.asp?ProvinceID="+ProvinceID;}
}
</script>
<script language="javascript">
...
window.location.href="areamodify_KH.asp?CityID="+CityID;}
else{
form1.town.value=0;}
}
</script>
  ...
  <tr>
    <td valign="top"><table width="400" height="271" cellpadding="-2" cellspacing="-2">
      ...
      <tr>
        ...
        <td valign="top">  
          <div align="center">
            <form ACTION="areamodify_KH.asp" METHOD="POST" name="form1">
            
            <table width="88%" height="142" border="1" cellpadding="-2"  cellspacing="-2"
			 bordercolor="#669999" bordercolordark="#FFFFFF">
              <tr>
                <td width="32%" height="27" bgcolor="#809EA4"><div align="center"
				 class="style1">省级名称： </div></td>
                <td width="68%">
				<%
...
%>
  <option value="<%=rs_city("ID")%>" <% if rs_city("ID")=cint(SelectID_C) then %> selected
   <%end if%>><%=rs_city("Name")%></option>
  <%
  rs_city.MoveNext()
Wend
