...
If (Request.Form("Province") <> "") Then 
  vProvince = Request.Form("Province")
End If
'查询用户所输入的省级名称是否包括下属地区
Set rs_father = Server.CreateObject("ADODB.Recordset")
sql_F = "SELECT * FROM DB_Area WHERE Father = " + Replace(vProvince, "'", "''") + ""
rs_father.open sql_F,conn,1,3
if not rs_father.eof then%>
	<script language="javascript">
	alert("该省份还包括下属地区，请先删除其下属地区！");
	window.close();
	</script>
<%else
	'判断用户输入的省份是否包括客户信息
	sql="delete from DB_area where ID="&request.Form("province")
	On error resume next  '设置错误陷阱
	set rs=conn.execute(sql)  '删除
	if err then %>
		<script language="javascript">
      ...
      <tr>
        ...
        <td valign="top">
          ...
          <div align="center">
            ...
            <table width="80%" height="147" border="1" cellpadding="-2"  cellspacing="-2" bordercolor="#669999" bordercolordark="#FFFFFF">
              <tr>
