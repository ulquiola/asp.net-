...
function ChangeItem(){
var SelectID=form1.Province.value;
window.location.href="del_area_T.asp?SelectID=2";
}
</script>
...
%>
  <option value="<%=(rs_province.Fields.Item("ID").Value)%>" <% if SelectID=rs_province("ID") then %>selected <%end if%>><%=(rs_province.Fields.Item("Name").Value)%></option>
  <%
