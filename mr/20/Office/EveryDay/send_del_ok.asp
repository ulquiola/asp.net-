<%@LANGUAGE="VBSCRIPT" CODEPAGE="936"%>
<!--#include file="../Conn/conn.asp" -->
<%
if session("ID")<>"" then
	Del="Delete from Tab_Send where ID="&session("ID")
	conn.execute(Del)
end if%>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<title>删除成功页面 </title>
<meta http-equiv="Content-Type" content="text/html; charset=gb2312">
<link href="../style.css" rel="stylesheet">
<style type="text/css">
<!--
body {
	margin-left: 0px;
	margin-top: 0px;
}
.style1 {color: #C60001}
.style3 {
	color: #C60001;
	font-size: 12pt;
	font-weight: bold;
}
.STYLE5 {
	font-size: 9pt;
	color: #FFFFFF;
}
-->
</style></head>
<body>
<table width="557" height="401" border="0" cellpadding="-2" cellspacing="-2" background="../Images/detail0.gif">
  <tr>
    <td width="817" height="349" valign="top"><table width="100%" height="102"  border="0" cellpadding="-2" cellspacing="-2">
      <tr>
        <td height="44">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="STYLE5">删除成功页面 </span></td>
      </tr>
      <tr>
        <td>&nbsp;</td>
      </tr>
    </table>      
      <form name="form1" method="post" action="">
        <table width="80%" height="169"  border="0" align="center" cellpadding="-2" cellspacing="-2">
          <tr>
            <td height="105"><div align="center" class="style1"></div>              
            <div align="center"><span class="style3">删除成功！</span></div></td>
          </tr>
          <tr>
            <td height="25" valign="top"><div align="center" class="style1"></div></td>
          </tr>
          <tr>
            <td><div align="center">
                <input name="myclose" type="button" class="Style_button" id="myclose" value="关闭" onClick="javascrip:opener.location.reload();window.close()">
            </div></td>
          </tr>
        </table>
      </form></td>
  </tr>
</table>
</body>
</html>
<%session("ID")=""%>