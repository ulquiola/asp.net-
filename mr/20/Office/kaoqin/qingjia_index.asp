...
sql="select * from Tab_qingjia order by ID desc"
rs.open sql,conn,1,3
%>
<%
if request.QueryString("id")<>"" then
yyy="update Tab_qingjia set state=1 where ID="&request.QueryString("id")
conn.execute(yyy)
end if
%>
    ...
    <td valign="top" background="../Images/qingjia.gif"> 
      ...
      <tr>
        <td width="691" height="52" valign="bottom"><table width="100" height="32" border="0">
          <tr>
            <td>&nbsp;<span class="STYLE3">请假登记</span></td>
          </tr>
        </table></td>
        <td width="71" valign="bottom"><a href="#" onClick="Javascript:window.open('qingjia_add.asp','','width=580,height=433')"><img src="../Images/waichuan.gif" width="56" height="21" border="0" /></a></td>
      </tr>
	  ...
	  %>
          <tr>
            ...
            <td width="185" class="STYLE1"><div align="center" class="STYLE1">
			  <% if len(k)>10 then%>
               <a href="#" onClick="Javascript:window.open('qingjia_xianxi.asp?ID=<%=rs("ID")%>','','width=456,height=459')"><%=replace(left(k,10)&"....",chr(13),"<br>")%></a>
              <%else%> 
              <a href="#" onClick="Javascript:window.open('qingjia_xianxi.asp?ID=<%=rs("ID")%>','','width=456,height=459')"><%=k%></a>
              <%end if %>
			</div></td>
	   ...
	   <%if rs("state")=1 then%>
        ...
        <% End If %>
        <%if rs("state")=0 then%>
        <a href="qingjia_index.asp?state=1&id=<%=rs("id")%>" class="STYLE1" onClick="return confirm('确定销假吗？')">销假</a>
        <% End If %>
		</td>
        </tr>
	<%
