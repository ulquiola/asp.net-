...
<table width="810" height="536" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td valign="top" background="../Images/waichu.gif"> 
							<table width="762" height="503" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td width="691" height="52">&nbsp;</td>
        <td width="71" valign="bottom"><a href="#" onClick="Javascript:window.open('waichu_add.asp','','width=580,height=433')"><img src="../Images/waichuan.gif" width="56" height="21" border="0" /></a></td>
      </tr>
	  ...
	  %>
          <tr>
            ...
            <td width="160" class="STYLE1"><div align="center" class="STYLE1">
			  <% if len(k)>10 then%>
               <a href="#" onClick="Javascript:window.open('waichu_xianxi.asp?ID=<%=rs("ID")%>','','width=456,height=459')"><%=replace(left(k,10)&"....",chr(13),"<br>")%></a>
              <%else%> 
              <a href="#"onClick="Javascript:window.open('waichu_xianxi.asp?ID=<%=rs("ID")%>','','width=456,height=459')"><%=k%></a>
              <%end if %>
			</div></td>
        ...
        <% End If %>
        <%if rs("state")=0 then%>
        <a href="waichu.asp?state=1&id=<%=rs("id")%>" onClick="return confirm('确定销假吗？')">销假</a>
        <% End If %>			</div></td>
          </tr>
	<%
	  rs.movenext
